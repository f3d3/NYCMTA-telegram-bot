...
async def gtfs_update(*args):
    ...
    if (len(args)==1 and context.job.name=='gtfs_daily_update') \
        or (len(args)==2 and (datetime.now()-last_gtfs_update).total_seconds()>=86400 and (os.path.isdir(dir)) and (os.path.isdir("cache/stop_times")) and (os.path.isdir("cache/trips"))) \
        or (len(args)==2 and (datetime.now()-last_gtfs_update).total_seconds()<86400 and (not os.path.isdir(dir) or (not os.path.isdir("cache/stop_times")) or (not os.path.isdir("cache/trips")))):
